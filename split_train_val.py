...
X, y = load_dataset(Path('data/split_dataset2/train'))
...
for idx_label, image_path in enumerate(X_train):
    ...
    output_folder = "data/data_to_train2/train/" + str("{:02d}".format(y_train[idx_label]))
...
for idx_label, image_path in enumerate(X_val):
    img = cv2.imread(str(image_path))
    output_folder = "data/data_to_train2/val/" + str("{:02d}".format(y_val[idx_label]))
    Path(output_folder).mkdir(parents=True, exist_ok=True)
    output_path = output_folder + "/" + image_path.stem + ".jpeg"
    cv2.imwrite(output_path, img)
