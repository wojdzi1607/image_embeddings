...
for idx_label, image_path in enumerate(X_train):
    ...
    output_folder = "data/split_dataset/split_dataset_390_NOMASK/train/" + str("{:02d}".format(y_train[idx_label]))
...
for idx_label, image_path in enumerate(X_test):
    img = cv2.imread(str(image_path))
    output_folder = "data/split_dataset/split_dataset_390_NOMASK/test/" + str("{:02d}".format(y_test[idx_label]))
    Path(output_folder).mkdir(parents=True, exist_ok=True)
